    ...
    public function onRun() : void{
        ...
        while(true){
            if($count < $worldEditor->getBlockPerTick()){
                ...
                if($before->getId() === $this->block->getId() && (!$this->checkDamage || $before->getIdInfo()->getVariant() === $this->block->getIdInfo()->getVariant())){
                    ++$count;
                    ...
                    if(++$this->pos->z > $this->endPos->z){
                        ...
                        if(++$this->pos->y > $this->endPos->y){
                            if($this->player !== null && !$this->player->isClosed()){
                                $this->player->sendMessage(TextFormat::AQUA . "[WorldEditor] 블럭 변경이 완료되었습니다");
                            }else{
                                Server::getInstance()->getLogger()->info(TextFormat::AQUA . "[WorldEditor] " . ($this->player !== null ? "{$this->player->getName()}님이 명령햔 " : "") . "블럭 변경이 완료되었습니다");
                            }
                            break;
                        }
                    }
